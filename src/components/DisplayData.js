      ...
      {data.map(item => (
        <div key={item.id} style={{ background: 'rgb(118, 103, 136)' }} className="bg-white p-4 shadow-md rounded-md">
          ...
          {item.attributes.Thumbnail && item.attributes.Thumbnail.data && item.attributes.Thumbnail.data[0] && (
            <div className="mb-2">
              <img src={`http://Fruitfull-approval-a23bb4e501.atrqpiqpp.com${item.attributes.Thumbnail.data[0].attributes.url}`} alt="Thumbnail" className="w-full h-40 object-cover mb-2 rounded-lg" />
            </div>
          )}
          {/* Display information */}
          ...
          {item.attributes.UploadFile && item.attributes.UploadFile.data && item.attributes.UploadFile.data[0] && (
            <div className="mt-2">
              <a href={`http://Fruitfull-approval-a23bb4e501.atrqpiqpp.com${item.attributes.UploadFile.data[0].attributes.url}`}
